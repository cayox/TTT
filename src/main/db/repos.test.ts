...
describe('projects', () => {
  it('seeds General and assigns new sessions to the default project', () => {
    const [general] = r.projects.list()
    expect(general).toMatchObject({ name: 'General', color: 'brass', archived: false })
    expect(r.sessions.start('manual', 1000).projectId).toBe(general.id)
    r.sessions.stop(2000)
    const b = r.projects.add({ name: ' Client B ', color: 'sage' })
    expect(b.name).toBe('Client B')
    r.settings.set({ defaultProjectId: b.id })
    expect(r.sessions.start('manual', 3000).projectId).toBe(b.id)
    r.sessions.stop(4000)
    expect(r.sessions.start('manual', 5000, general.id).projectId).toBe(general.id)
  })
  it('falls back when the default is archived and maps SSIDs case-insensitively', () => {
    const [general] = r.projects.list()
    const b = r.projects.add({ name: 'B', color: 'teal' })
    r.settings.set({ defaultProjectId: b.id, ssidProjects: { 'Office-5G': general.id } })
    expect(r.projects.forSsid('office-5g')).toBe(general.id)
    expect(r.projects.forSsid('Cafe')).toBe(b.id)
    r.projects.update(b.id, { archived: true })
    expect(r.projects.defaultId()).toBe(general.id)
    expect(r.projects.list(false).map((p) => p.id)).toEqual([general.id])
  })
  it('usage counts sessions; delete unassigns them', () => {
    const b = r.projects.add({ name: 'B', color: 'plum' })
    const s = r.sessions.add({ startTs: 1, endTs: 2, source: 'manual', projectId: b.id })
    expect(r.projects.usage()[b.id]).toBe(1)
    r.projects.remove(b.id)
    expect(r.sessions.list(0, 10).find((x) => x.id === s.id)?.projectId).toBeNull()
  })
})
