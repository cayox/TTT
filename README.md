...
- **One click to start.** Start or pause from the window, the menu bar or <kbd>⌘</kbd><kbd>↩</kbd>. The menu bar ring fills up as you work through the day and shows a dot while the timer runs.
...
To read the Wi-Fi name, macOS asks for Location access the first time automatic tracking looks up the network. Without it TTT falls back to recognizing the network by its router.
...
```
src/
  main/       Electron main process: window, tray, IPC, SQLite, Wi-Fi watcher, export
...
build/        App icon sources and the Wi-Fi helper build script
```
...
The app version comes from the tag, so `package.json` does not need a bump. Tags with a suffix such as `-beta.1` are published as pre-releases.
