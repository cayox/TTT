...
- [x] 5.4 App icon, packaging (electron-builder dmg)
...
- 2026-09-19: Redesign: sand-slate theme + grain, Fraunces display font, top tab bar with live status pill (sidebar removed), History month heatmap, Stats hierarchy, grouped Settings, keyboard shortcuts (⌘1-4, ⌘↩, Space, ←/→). Startup error screen (public/boot-error.js).
- 2026-09-19: Projects: migration #3 (projects table, sessions.project_id, seeds "General" and backfills), settings.defaultProjectId + ssidProjects, IPC projects:* / stats:projects / wifi:current, watcher wired in main (Wi-Fi sessions use the SSID's project). UI: project picker in status pill + Today, per-session project in Today/History, by-project in Today/Stats, Projects section in Settings. App icon: build/icon.svg -> build/make-icon.sh -> icon.png/icon.icns (4.3 UI done).
- 2026-09-19: Quick edit on Today sessions (shared components/SessionEditor.tsx with start/end date+time, project, note; delete; add), also used by History (running sessions editable). settings.startingBalance (signed min) added to stats:balance and all-time stats:range balance; set in Settings > Schedule.
- 2026-09-19: Month export (History > Export): A4 timesheet PDF with signature lines (main/export, hidden window printToPDF, save dialog, Open/Finder/Share), WhatsApp summary (shared/report.ts whatsappText; copy to clipboard or whatsapp:// deep link). settings.exportName.
- 2026-09-19: Network detection: SSID is redacted on macOS 26 without Location permission, so networks can also be recognized by default-router MAC (route + arp, no permission). settings.networkRouters name->MAC; watcher matchWorkNetwork (SSID first, router fallback); Settings: Use current network asks for a name when SSID hidden, per-row Link to current network, Connected badge. npm run dev now uses --watch.
- 2026-09-19: Toasts (lib/toast.tsx: ToastProvider/useToast, bottom-right stack, success/error/warning/info, actions, hover-paused timer bar, dedupe by id). All inline status text moved to toasts; deletes (sessions, unused projects, networks, day markers) are immediate with Undo; SessionEditor marks invalid fields red and explains via toast; lib/sessions.ts useSessionActions.
- 2026-09-19: Monthly goal: settings.monthlyGoalMin (0=off); shared/goal.ts monthGoal/monthTotals (+tests); IPC stats:goal, stats:months. Today goal block (live while running), Stats 6-month goal card, Settings section with Match schedule.
- 2026-09-19: Brand + release: Wordmark/BrandMark (components/ui/Brand.tsx) in the top bar; tray glyph is a progress ring (today's worked / expected, center dot while running, monospaced title). First-run onboarding (components/Onboarding.tsx: welcome, work week sliders, projects, Wi-Fi + login, ready) gated by settings.onboarded (migration #4 marks existing installs done), reopenable from Settings > App. Packaging: electron-builder.yml (ad-hoc signed, dmg+zip arm64/x64, ssid-helper via npm run helper), `npm run dist`; .github/workflows/release.yml publishes a release on v* tags. README with dark screenshots in docs/screenshots.
- 2026-09-19: Monthly goal became monthly contract hours with flextime (Gleitzeit): shared/hours.ts (monthHours, flexNow, contractMinutes; pace follows the schedule), settings monthlyHoursMin / monthlyHoursByProject / projectMonthlyMin (migration #5 renames monthlyGoalMin), IPC stats:month. Today shows hours left, per workday, flextime, per-project rows. Settings reorganized into panes (Work time, Projects, Automatic tracking, General) with shared components/WeekEditor.tsx (also used by onboarding). Header wordmark "TTT - Time Tracking Tool". Ring glow no longer clipped.
